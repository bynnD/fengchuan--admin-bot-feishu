...
def extract_fields_from_file(file_content, file_name, form_opts, get_token):
    ...
    prompt = (
        ...
        f"- business_type: 业务类型。仅从合同/协议类文件中提取。必须从合同正文中直接读取，尤其是合同第一页、标题、服务内容等明确写出的业务/产品名称。"
        f"如合同写「小蓝词」则返回小蓝词，写「广告」则返回广告，写「技术服务」则返回技术服务。不要推测、不要使用合同正文中未出现的词汇。"
        f"结算单、对账单、银行流水、订单明细、截图等非合同类文件不要返回 business_type。\n"
