...
APPROVAL_USAGE_GUIDE = {
    # 用印：上传文件后自动识别，选完选项即可创建
    "用印申请单": ("上传需盖章的文件（Word/PDF/图片），我会自动识别并生成工单", "我要盖章", False),
    # 开票：发票类型、开票项目必填
    "开票申请单": ("发送后按提示上传结算单和合同；必须说明发票类型和开票项目", "我要开增值税发票，发票内容是技术服务费", False),
    # 采购：采购事由、采购类别、期望交付时间
    "采购申请": ("说明采购事由、采购类别、期望交付时间", "采购一台笔记本 ThinkPad X1 8000元，下周一到货", False),
    # 外出：时间、地点、事由（支持自动创建，若为报备单则需在飞书后台添加审批节点）
