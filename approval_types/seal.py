...
def extract_fields_from_file(file_content, file_name, form_opts, get_token):
    ...
    extra = "" if has_content else (
        ...
        "请据此推断：company 从文件名中的公司名匹配上述选项（风船、扇贝等），seal_type 结算单/对账类用合同章、合同/协议类用合同章或公章，reason 填合同/协议/结算单名称，document_type 从文件名推断业务类型。"
    )
    prompt = (
        f"根据合同/文件内容识别用印申请信息。\n\n{combined}\n\n"
        ...
        f"- seal_type: 结算单、对账单、月结单等用合同章；合同/协议类用合同章或公章；财务类用财务章\n"
