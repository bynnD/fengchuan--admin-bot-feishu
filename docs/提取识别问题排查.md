...
- **原因**：应用未开通 OCR 权限、token 失效、图片过大、**请求频率限制**（request trigger frequency limit）
- **处理**：
  - 在飞书开放平台为应用开通「光学字符识别」权限
  - 频率限制：代码已内置重试（2s/4s/8s 退避）及 PDF 多页间隔 1 秒；若仍频繁触发，可适当提高 `file_extraction.py` 中 `OCR_RETRY_DELAYS` 或页间 `time.sleep`
